                        ...
                        @forelse ($invoices as $invoice)
                            <tr>
                                <td class="border border-gray-300 px-4 py-2">
                                    <a href="{{ route('invoices.show', $invoice->id) }}" class="underline">
                                ...
                                <td class="border border-gray-300 px-4 py-2">R {{ number_format($invoice->total, 2) }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ ucfirst($invoice->status) }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ $invoice->issue_date->format('d M Y') }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ $invoice->due_date->format('d M Y') }}</td>
                                <td class="border border-gray-300 px-4 py-2">
                                    <div class="inline-flex gap-2 items-center rounded-md shadow-sm">
                                        ...
                                        <form method="POST" action="{{ route('invoices.destroy', $invoice->id) }}"
                                            onsubmit="return confirm('Delete invoice {{ $invoice->invoice_number }}?')" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
