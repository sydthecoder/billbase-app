                ...
                <table id="example" class="table-auto table-style w-full mx-auto">
                    <thead>
                        <tr>
                            ...
                            <th class="border border-gray-300 px-4 py-2">Expires</th>
                        ...
                        @forelse ($quotes as $quote)
                            <tr>
                                <td class="border border-gray-300 px-4 py-2">
                                    <a href="{{ route('quotes.show', $quote->id) }}" class="underline">
                                        {{ $quote->quote_number }}
                                    </a>
                                </td>
                                <td class="border border-gray-300 px-4 py-2">{{ $quote->title ?? '—' }}</td>
                                <td class="border border-gray-300 px-4 py-2">
                                    {{ $quote->customer->first_name }} {{ $quote->customer->last_name }}
                                </td>
                                <td class="border border-gray-300 px-4 py-2">{{ $quote->issue_date->format('d M Y') }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ $quote->expires_at->format('d M Y') }}</td>
                                <td class="border border-gray-300 px-4 py-2">R {{ number_format($quote->total, 2) }}</td>
                                <td class="border border-gray-300 px-4 py-2">
                                    @php
                                        $statusStyles = [
                                        ...
                                        $style = $statusStyles[$quote->status] ?? 'bg-gray-100 text-gray-700';
                                    @endphp
                                ...
                                <td class="border border-gray-300 px-4 py-2">
                                    <div class="inline-flex gap-2 items-center rounded-md shadow-sm">
                                        ...
                                        <form method="POST" action="{{ route('quotes.destroy', $quote->id) }}"
                                              onsubmit="return confirm('Delete quote {{ $quote->quote_number }}?')" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
